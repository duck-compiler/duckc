...
fn simple_function() {
    ...
    let Item::Function(func) = &source_file.items[0] else { panic!() };
...
fn type_alias() {
    ...
    let Item::TypeAlias(type_alias) = &source_file.items[0] else { panic!() };
...
fn struct_decl() {
    ...
    let Item::Struct(struct_decl) = &source_file.items[0] else { panic!() };
...
fn use_duck() {
    ...
    let Item::Use(UseDecl::Duck(segs, _)) = &source_file.items[0] else { panic!() };
...
fn use_go() {
    ...
    assert!(matches!(&source_file.items[0], Item::Use(UseDecl::Go(_, _))));
}

#[test]
fn extension_decl() {
    let source_file = parse_ok("extend Int with impl { fn double() -> Int { 0 } }");
    let Item::Extension(extension) = &source_file.items[0] else { panic!() };
...
fn expr_float() {
    ...
    let ExprKind::Float(float_val) = expr.kind else { panic!() };
...
fn expr_block() {
    ...
    let ExprKind::Block(stmts) = expr.kind else { panic!() };
...
#[test]
fn expr_return() {
    assert!(matches!(parse_expr_ok("return 42").kind, ExprKind::Return(Some(_))));
    assert!(matches!(parse_expr_ok("return").kind,    ExprKind::Return(None)));
}
...
fn expr_duck_lit() {
    ...
    let ExprKind::DuckLit(fields) = expr.kind else { panic!() };
...
fn expr_array() {
    ...
    let ExprKind::Array(elems) = expr.kind else { panic!() };
...
fn expr_tuple() {
    let expr = parse_expr_ok("(1, 2)");
    let ExprKind::Tuple(elems) = expr.kind else { panic!() };
...
fn expr_match() {
    ...
    let ExprKind::Match { arms, else_arm, .. } = expr.kind else { panic!() };
...
fn expr_let_decl() {
    ...
    let ExprKind::Let { name, .. } = &expr.kind else { panic!() };
...
#[test]
fn expr_defer_async() {
    assert!(matches!(parse_expr_ok("defer foo()").kind, ExprKind::Defer(_)));
    assert!(matches!(parse_expr_ok("async foo()").kind, ExprKind::Async(_)));
}
...
fn type_primitives() {
    ...
    assert!(matches!(&source_file.items[0], Item::TypeAlias(type_alias) if matches!(type_alias.type_expr.desc, TypeDescription::Int)));
}
...
fn type_array() {
    ...
    let Item::TypeAlias(type_alias) = &source_file.items[0] else { panic!() };
    assert!(matches!(&type_alias.type_expr.desc, TypeDescription::Array(_)));
}
...
fn type_fun() {
    ...
    let Item::TypeAlias(type_alias) = &source_file.items[0] else { panic!() };
    assert!(matches!(&type_alias.type_expr.desc, TypeDescription::Fun { .. }));
}
...
fn type_duck() {
    ...
    let Item::TypeAlias(type_alias) = &source_file.items[0] else { panic!() };
    assert!(matches!(&type_alias.type_expr.desc, TypeDescription::Duck(_)));
}
...
fn type_ref() {
    ...
    let Item::TypeAlias(type_alias) = &source_file.items[0] else { panic!() };
    assert!(matches!(&type_alias.type_expr.desc, TypeDescription::Ref(_)));
}
...
#[test]
fn program_test() {
    let source_file = parse_ok("fn main() {\n    const x = fn (f: String) {\n        let y = 10;\n    }\n}");

    // one top-level item: `fn main`
    assert_eq!(source_file.items.len(), 1);
    let Item::Function(main_fn) = &source_file.items[0] else { panic!("expected fn") };
    ...
    let ExprKind::Block(body_stmts) = &main_fn.body.kind else { panic!("expected block") };
    assert_eq!(body_stmts.len(), 1, "body should have exactly one statement");
    let ExprKind::Const { name, type_ann, value } = &body_stmts[0].kind else {
    ...
    let ExprKind::Lambda { is_mut, params, return_type, body } = &value.kind else {
        panic!("expected lambda")
    };
    assert!(!is_mut);
    assert_eq!(params.len(), 1);
    assert_eq!(params[0].name.value, "f");
    assert!(matches!(&params[0].type_expr.desc, TypeDescription::String(_)));
    assert!(return_type.is_none());

    // lambda body: block with `let y = 10` then implicit unit from trailing `;`
    let ExprKind::Block(lambda_stmts) = &body.kind else { panic!("expected block") };
    assert_eq!(lambda_stmts.len(), 2, "let + implicit unit from trailing semicolon");
    let ExprKind::Let { name: y_name, is_mut: y_mut, type_ann: y_ann, value: y_val } = &lambda_stmts[0].kind else {
