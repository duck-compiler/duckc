    ...
    I: BorrowInput<'src, Token = Token, Span = SS>,
{
    recursive(
        |p: Recursive<dyn Parser<'_, _, Spanned<TypeExpr>, extra::Err<Rich<'src, Token, SS>>>>| {
            ...
            let function_fields = duck_fields
                .clone();
            ...
            let function = just(Token::ControlChar('('))
                ...
                .then_ignore(just(Token::ControlChar('>')))
                .then(p.clone())
                .map(|(fields, return_type)| TypeExpr::Fun(
                    fields.iter()
...
pub mod tests {
    ...
    fn strip_spans(spanned_type_expr: Spanned<TypeExpr>) -> Spanned<TypeExpr> {
        ...
        let stripped_expr = match expr {
            TypeExpr::Struct(s) => TypeExpr::Struct(Struct {
                fields: s.fields.into_iter().map(|field| {
                    Field {
                        name: field.name,
            ...
            TypeExpr::Duck(d) => TypeExpr::Duck(Duck {
                fields: d.fields.into_iter().map(|field| {
                    Field {
                        name: field.name,
                        type_expr: strip_spans(field.type_expr),
                    }
            ...
            TypeExpr::Tuple(t) => TypeExpr::Tuple(
                t.into_iter().map(strip_spans).collect()
            ),
            TypeExpr::Fun(params, return_type) => TypeExpr::Fun(
                params.into_iter().map(|(name, param_type_expr)| {
                    (name, strip_spans(param_type_expr))
                }).collect(),
                return_type.map(|rt_box| Box::new(strip_spans(*rt_box)))
            ),
            TypeExpr::Or(variants) => TypeExpr::Or(
                variants.into_iter().map(strip_spans).collect()
            ),
            other => other,
        };
    ...
    fn assert_type_expression(input_str: &str, expected_expr: TypeExpr) {
        ...
        let parse_result = type_expression_parser()
            .parse(make_input(empty_range(), &tokens));

        assert!(
            !parse_result.has_errors(),
            "parsing errors for \"{}\": {:?}",
            input_str, parse_result.errors()
                .map(|err| err.to_string())
                .collect::<Vec<_>>(),
        );

        assert!(parse_result.has_output(), "parser produced no output for \"{}\"", input_str);

        let parsed = parse_result.into_output().unwrap();

        let stripped_parsed = strip_spans(parsed);

        assert_eq!(stripped_parsed.0, expected_expr, "mismatch for \"{}\"", input_str);
    }
    ...
    fn test_assert_type() {
        ...
        assert_type_expression(
            "(param1: TypeName,) -> ()",
            TypeExpr::Fun(
                vec![("param1".to_string().into(), TypeExpr::TypeName(false, "TypeName".to_string()).into_empty_span())],
        ...
        assert_type_expression(
            ...
            TypeExpr::Fun(
                ...
                Some(Box::new(TypeExpr::TypeName(true, "MyResult".to_string()).into_empty_span()))
            )
        );
        ...
        assert_type_expression(
            ...
            TypeExpr::Fun(
                vec![],
                Some(Box::new(TypeExpr::Tuple(vec![
        ...
        assert_type_expression(
            ...
            TypeExpr::Fun(
                vec![("x".to_string().into(), TypeExpr::Int.into_empty_span())],
                Some(Box::new(TypeExpr::Go("fmt.Stringer".to_string()).into_empty_span()))
            )
        );
            ...
            TypeExpr::Fun(
                vec![
                    (
                        ...
                        TypeExpr::Struct(Struct {
                            fields: vec![Field::new("id".to_string(), TypeExpr::Int.into_empty_span())],
                        }).into_empty_span(),
                    )
                ],
                Some(Box::new(TypeExpr::Struct(Struct {
                    fields: vec![Field::new("success".to_string(), TypeExpr::Bool.into_empty_span())],
                }).into_empty_span()))
            )
        );
        ...
        assert_type_expression(
            ...
            TypeExpr::Fun(
                ...
                Some(Box::new(TypeExpr::Tuple(vec![
                    TypeExpr::Int.into_empty_span(),
                    TypeExpr::Duck(Duck {
                        fields: vec![Field::new("val".to_string(), TypeExpr::Char.into_empty_span())],
                    }).into_empty_span(),
                ]).into_empty_span()))
            )
        );
        ...
        assert_type_expression("go sync.WaitGroup", TypeExpr::Go("sync.WaitGroup".to_string()));
        ...
        assert_type_expression(
            ...
            TypeExpr::Struct(Struct {
                fields: vec![
                    Field::new("nested".to_string(), TypeExpr::Struct(Struct {
                        fields: vec![Field::new("inner".to_string(), TypeExpr::Float.into_empty_span())],
                    }).into_empty_span()),
                ],
            })
        );
        ...
        assert_type_expression(
            "duck { name: String }",
            TypeExpr::Duck(Duck {
                fields: vec![Field::new("name".to_string(), TypeExpr::String.into_empty_span())],
            })
        );
        ...
        assert_type_expression(
            ...
            TypeExpr::Duck(Duck {
                fields: vec![
                    Field::new("nested".to_string(), TypeExpr::Duck(Duck {
                        fields: vec![Field::new("value".to_string(), TypeExpr::Char.into_empty_span())],
                    }).into_empty_span()),
                ],
            })
        );
        ...
        assert_type_expression("::GlobalType", TypeExpr::TypeName(true, "GlobalType".to_string()));
        assert_type_expression("Module::MyType", TypeExpr::TypeName(false, "Module_MyType".to_string()));
        assert_type_expression("::Module::MyType", TypeExpr::TypeName(true, "Module_MyType".to_string()));
        assert_type_expression("::Module::SubModule::MyType", TypeExpr::TypeName(true, "Module_SubModule_MyType".to_string()));
        ...
        assert_type_expression(
            ...
            TypeExpr::Or(vec![
                ...
                TypeExpr::Struct(Struct {
                    fields: vec![Field::new("y".to_string(), TypeExpr::String.into_empty_span())],
                }).into_empty_span(),
            ])
        );
    }
