...
use crate::parse::{function_parser2::function_definition_parser2, lexer::{lexer2, Token}, make_input, source_file_parser::{SourceFile, SourceUnit}, type_parser2::type_definition_parser2, use_statement_parser2::{self, use_statement_parser2}};
...
fn module_descent(name: String, current_dir: PathBuf) -> SourceFile {
        ...
        && mod_dir.metadata().unwrap().is_dir()
    {
        ...
        let lex = lexer2().parse(&src_text).unwrap();
        let parse = source_file_parser2(current_dir.clone()).parse(make_input((0..src_text.len()).into(), &lex)).unwrap();
        parse
    }
}

pub fn source_file_parser2<'src, I>(p: PathBuf) -> impl Parser<'src, I, SourceFile, extra::Err<Rich<'src, Token>>>
where I: BorrowInput<'src, Token = Token, Span = SimpleSpan>
{
    let p = Box::leak(Box::new(p));
    recursive(|e| {
        choice((
            use_statement_parser2().map(SourceUnit::Use),
            type_definition_parser2().map(SourceUnit::Type),
            function_definition_parser2().map(SourceUnit::Func),
            just(Token::Module)
